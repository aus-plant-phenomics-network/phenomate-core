...
# Example filename: 2026-05-05_16-09-58_681639_+0930_station5mayr3-CaptureSoftware.json


from phenomate_core.get_version import get_task_logger
shared_logger = get_task_logger(__name__)

class CaptureSoftwarePreprocessor(BasePreprocessor[Path]):
    """
    
    We have special processing in the phenomate/backend/activity/tasks.py remove_task() method 
    ...
    def extract(self, **kwargs: Any) -> None:
        ...
        shared_logger.info(f"CaptureSoftwarePreprocessor extract(): file_part = {file_part}")
        # self.images = path_objects

    def copy_extra_files(self, fpath: Path) -> None:
        """
        Don't copy any extra files with CaptureData data.
        """
    ...
    def save(
        ...
        **kwargs: Any,
        ...
        """
        We do not have to save the input data for CaptureSoftware, as the CaptureSoftware file is copied 
        ...
        # Print elapsed time
        shared_logger.info(f"CaptureSoftwarePreprocessor: save(): path = {path}")
