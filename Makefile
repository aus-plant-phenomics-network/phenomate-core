...
.PHONY: compile-oak
compile-oak:
	@uv run ${PROTOC} -I ${OAK_SRC} --python_out ${OAK_SRC} --mypy_out ${OAK_SRC} ${OAK_FILE}

.PHONY: compile-jai
compile-jai:
	@uv run ${PROTOC} -I ${JAI_SRC} --python_out ${JAI_SRC} --mypy_out ${JAI_SRC} ${JAI_FILE}

.PHONY: compile-lidar
compile-lidar:
	@uv run ${PROTOC} -I ${LIDAR_SRC}/ --python_out ${LIDAR_SRC} --mypy_out ${LIDAR_SRC} ${LIDAR_FILE}
